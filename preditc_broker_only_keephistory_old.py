...
for id in df.index:
    ...
    import os
    try:
        if os.path.isfile(f"./transdb/phold_{mainid}_{subid}_{stockid}.csv"):
            hs = pd.read_csv(f"./transdb/phold_{mainid}_{subid}_{stockid}.csv")
        else:
    ...
    close.to_csv(f"./transdb/phold_{mainid}_{subid}_{stockid}.csv")
    close.to_csv(f"./transdb_history/phold_{mainid}_{subid}_{stockid}_{datetime.datetime.now().date()}.csv")
    fig = plt.figure(figsize = (10, 8),dpi=80)
    ...
    plot.plot(s2)
    plot.savefig(f"./transdb/phold_{mainid}_{subid}_{stockid}.png")
    plot.savefig(f"./transdb_history/phold_{mainid}_{subid}_{stockid}_{datetime.datetime.now().date()}.png")
    #plot.show()

    s1 = close[[stockid,'bs','trans2','hold_price2']]
    fig1 = bs_chart(s1,stockid)
    fig1.savefig(f"./transdb/phold_{mainid}_{subid}_{stockid}_bs.png")
    fig1.savefig(f"./transdb_history/phold_{mainid}_{subid}_{stockid}_{datetime.datetime.now().date()}_bs.png")



df = pd.DataFrame(data=profitResults,columns=col)
df.to_csv(f"./transdb/phold_result_{datetime.datetime.now().date()}.csv")
print(df)
#close.to_csv('phold.csv')
