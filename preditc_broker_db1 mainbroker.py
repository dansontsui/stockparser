...
for Mborkeridx in Mborkerdfidx.index:
    ...
    try:
        for stkid in stkidlist.index:
            ...
            scount += 1
            sql = f"select * from pdit_main_data where mainid='{mainid}' and stockid='{stockid}' and update_date='{date.strftime('%Y-%m-%d')}'"
            ...
            if len(s1) !=0:
                ...
                print(f"{mainid}{stockid} - {scount}/{len(stkidlist)}-t:{round((t4.timestamp()-t3.timestamp()),2)}-skip")
                continue
            try:
            ...
            brokerdata  = merge_sell_buy_data(mainid,'',stockid,pd.to_datetime(datetime.date(2022,3,1)))
            ...
            close.head(30)
            #close.to_csv('phold.csv')
            close.to_csv(f"./analysis/mainbroker/phold_{mainid}_{stockid}.csv")
            #cacl case2 profit
            close['case2profit'] = 0
            ...
            df.to_sql('pdit_main_data', conn,if_exists='append', index=False)
            #process_sql(df,sql,False)
            t2 = datetime.datetime.now()
    except Exception as e:
        print(f"excep {mainid} {stockid} -{str(e)}")
...
s1 = pd.read_sql(sql,conn)
s1.to_csv(f"./predict/mainbroker/pdit_{date.strftime('%Y-%m-%d')}.csv")