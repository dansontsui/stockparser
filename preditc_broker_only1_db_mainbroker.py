...
def bs_chart(s1,stockid,fn):
    ...
    pretrans=''
    for idx in s1.index:
        if s1['bs'][idx] == -1 and s1[fn][idx]==1:
            useszstr.append('s')
            pretrans = 's'
        elif s1['bs'][idx] == 1 and s1[fn][idx]==1:
            useszstr.append('b')
            pretrans = 'b'
        elif s1[fn][idx]==-1:
            if pretrans == 's':
    ...
    plt.savefig(f"./analysis/mainbroker/phold_{mainid}_{subid}_{stockid}_bs{fn}.png")
    plt.show()
...
print(df)
close.to_csv(f"./analysis/mainbroker/phold_{mainid}_{subid}_{stockid}.csv")
plot.plot(close['pred'])
s1 = close[stockid]
s2 = s1[s1 != 0]
plot.plot(s2)
plot.savefig(f"./analysis/mainbroker/phold_{mainid}_{subid}_{stockid}.png")
#plot.show()

s1 = close[[stockid,'bs','trans2','hold_price2']]
bs_chart(s1,stockid,'trans2')
